...
import { createPanelBlog } from '@/services/blogService';
...
const CreateBlogModal = ({ show, onClose, onCreate }: Props) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      const data = { ...values, thumbnail: uploadedThumbnail.filename };
      const [err, createdBlog] = await createPanelBlog(data);
      setLoading(false);
  ...
  const { errors, touched, handleSubmit, getFieldProps, resetForm, values, setFieldValue } = formik;
  ...
  return (
    <Modal title="Blog Yazısı Ekle" size="large" show={show} onClose={onClose}>
